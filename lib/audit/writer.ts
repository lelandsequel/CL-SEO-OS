/**
 * SEO OS++ Phase 0 Audit Writer
 * 
 * Writes audit artifacts: audit.json, audit.md, audit-summary.md
 */

import { AuditResult, Finding, AuditScores } from "./types";
...
function generateScoreSection(scores: AuditScores): string {
    let md = `## Audit Scores\n\n`;
...
export function generateAuditMarkdown(result: AuditResult): string {
    let md = `# SEO Audit Report: ${result.metadata.businessName}\n\n`;
    md += `**Target URL**: ${result.metadata.targetUrl}\n`;
    md += `**Audit Date**: ${result.metadata.auditedAt}\n`;
    ...
    md += `\n---\n\n*Generated by SEO OS++ Phase 0 Audit*\n`;

    return md;
}

// ═══════════════════════════════════════════════════════════════
// AUDIT SUMMARY (1-PAGER)
// ═══════════════════════════════════════════════════════════════

export function generateAuditSummary(result: AuditResult): string {
    let md = `# Audit Summary: ${result.metadata.businessName}\n\n`;
    ...
    result.topBlockers.slice(0, 5).forEach((f, i) => {
        md += `${i + 1}. ${severityEmoji(f.severity)} **${f.title}** — ${f.description.split('.')[0]}.\n`;
    });
    md += `\n`;
    ...
    md += `\n---\n\n*${result.metadata.pagesCrawled} pages analyzed on ${result.metadata.auditedAt.split('T')[0]}*\n`;
...
export function generateAuditJson(result: AuditResult): string {
